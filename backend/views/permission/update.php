...
<form class="layui-form" action="">
    ...
    <input type="hidden" name="PermissionForm[id]" value="<?= $info['id'] ?>" readonly/>
        ...
        <div class="layui-input-block">
            <select name="PermissionForm[fid]" lay-verify="required">
                ...
                <?php foreach($lists as $list){ ?>
                    <option value="<?= $list['id'] ?>" <?php if($info['fid']==$list['id']){ ?> selected="" <?php } ?>   ><?= $list['description'] ?></option>
                <?php } ?>
            </select>
        </div>
    </div>
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="PermissionForm[name]" lay-verify="required|menu_url" autocomplete="off" placeholder="请输入权限名" value="<?= $info['name'] ?>" class="layui-input">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">权限描述</label>
        <div class="layui-input-block">
            <input type="text" name="PermissionForm[description]" lay-verify="required|menu_name" autocomplete="off" placeholder="请输入权限描述" value="<?= $info['description'] ?>" class="layui-input">
        </div>
    </div>
    <div class="layui-form-item">
        <div class="layui-input-block">
    ...
    layui.use(['form'], function(){
        ...
        form.on('submit(imagecretae)', function(data){
            $.ajax({
                type:'POST',
                async: false,
                url: "<?= Url::toRoute(['permission/update']) ?>",
                data:data.field,
